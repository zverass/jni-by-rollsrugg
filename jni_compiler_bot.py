...
BOT_TOKEN = "8598894267:AAG8tUv3nDG1gjrPqLudy7lUc9FNlr8al5s"  # Получи от @BotFather
...
async def check_subscription(user_id: int, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    ИСПРАВЛЕННАЯ ВЕРСИЯ - использует CHANNEL_ID + fallback на username
    """
    try:
        member = None
        
        # ========== СТРАТЕГИЯ 1: Сначала пробуем по числовому ID (НАДЕЖНЕЕ) ==========
        if CHANNEL_ID and CHANNEL_ID != -1001234567890:  # Проверяем что ID задан
            try:
                logger.info(f"🔍 Попытка 1️⃣ : Проверка подписки {user_id} по CHANNEL_ID: {CHANNEL_ID}")
                member = await context.bot.get_chat_member(CHANNEL_ID, user_id)
                logger.info(f"✅ Способ 1️⃣ сработал! Статус: {member.status}")
            except Exception as e1:
                logger.warning(f"⚠️ Способ 1️⃣ (по ID) не сработал: {type(e1).__name__}: {e1}")
                member = None
        
        # ========== СТРАТЕГИЯ 2: Если не сработало, пробуем по username ==========
        if member is None:
            try:
                channel_check = CHANNEL_USERNAME.lstrip('@')
                logger.info(f"🔍 Попытка 2️⃣ : Проверка подписки {user_id} по username: @{channel_check}")
                member = await context.bot.get_chat_member(f"@{channel_check}", user_id)
                logger.info(f"✅ Способ 2️⃣ сработал! Статус: {member.status}")
            except Exception as e2:
                logger.error(f"❌ Способ 2️⃣ (по username) тоже не сработал: {type(e2).__name__}: {e2}")
                logger.error(f"   Проверьте что CHANNEL_ID и CHANNEL_USERNAME установлены правильно!")
                logger.error(f"   CHANNEL_ID = {CHANNEL_ID}")
                logger.error(f"   CHANNEL_USERNAME = {CHANNEL_USERNAME}")
                return False
        ...
        logger.info(f"📊 ИТОГ: Пользователь {user_id}, статус: {member.status} -> {'✅ ПОДПИСАН' if is_subscribed else '❌ НЕ ПОДПИСАН'}")
        ...
        logger.error(f"   Пожалуйста проверьте логи выше для деталей")
        return False
